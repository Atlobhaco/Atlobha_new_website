...
import { PAYMENT_METHODS, VEHICLE_PRICING } from "@/constants/enums";
import {
  generateSignature,
...
const CarPricingCheckoutSummary = forwardRef(
  (
    {
      setOpenAddMobile,
    ...
    const requestData = {
      ...
      return_url: `${process.env.NEXT_PUBLIC_PAYFORT_RETURN_GIFT_CARD}?carPricing=true`,
    };
    ...
    const handlePayWithTamara = async () => {
      ...
      const res = await fetch("/api/tamara/create-order", {
        ...
        body: JSON.stringify({
          ...
          items: [
            {
              reference_id: carPricing?.brand?.name,
              sku: carPricing?.brand?.name,
              name: carPricing?.brand?.name,
              quantity: 1,
              type: "Digital",
              total_amount: {
        ...
        body: JSON.stringify({
          payment: {
            ...
            order: {
              ...
              items: [
                {
                  title: carPricing?.brand?.name,
                  quantity: 1,
                  unit_price: calculateReceiptResFromMainPage?.amount_to_pay,
                  category: carPricing?.brand?.name,
                },
              ],
            },
            buyer_history: {
      ...
      const res = await fetch("/api/mis/create-order", {
        ...
        body: JSON.stringify({
          ...
          orderDetails: {
            items: [
              {
                nameArabic: carPricing?.brand?.name,
                nameEnglish: carPricing?.brand?.name,
                quantity: 1,
                unitPrice: calculateReceiptResFromMainPage?.amount_to_pay,
              },
            ],
          },
    ...
    return (
      <Box sx={{ pt: 1 }}>
        ...
        <Box className="d-flex justify-content-between mb-2">
          ...
          <Box sx={text}>
            {calculateReceiptResFromMainPage?.total_price} {riyalImgBlack()}
          </Box>
        </Box>
        {/* vat  percentage */}
        <Box sx={vat}>
        ...
        {/* hint for price back */}
        <Box sx={vat}>{t.priceWillBeBack}</Box>
        <SharedBtn
          disabled={
            loadPayRequest ||
